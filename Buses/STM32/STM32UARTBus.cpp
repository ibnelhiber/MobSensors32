...
void STM32UARTBus::UARTSetup()
{
    ...
    gpioInit.Alternate = enable_uart_clock_map.at(m_uartPort).portMacro;

    HAL_GPIO_Init(get_tx().gpioPort, &txInit);

    GPIO_InitTypeDef rxInit{};
    txInit.Pin = get_rx().pin;
    gpioInit.Mode = GPIO_MODE_AF_OD;
    gpioInit.Pull = GPIO_PULLUP;
    gpioInit.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    gpioInit.Alternate = enable_uart_clock_map.at(m_uartPort).portMacro;
...
{
    USART_TypeDef* tx_port = uart_pin_options_map.at(get_tx()).i2cPort;
    USART_TypeDef* rx_port = uart_pin_options_map.at(get_rx()).i2cPort;

    if(tx_port == rx_port && m_usedUARTPorts.find(tx_port) == m_usedI2CPorts.end())
    {
        m_uartPort->Instance = port;
        m_usedUARTPorts.insert(port);
        enable_uart_clock_map.at(port).enableUART();
        return true;
    }
...
bool STM32UARTBus::ReadFromRegister(
